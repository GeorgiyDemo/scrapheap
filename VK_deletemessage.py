...
def EditMessage(message_id):
	...
	if user_id == api.users.get(user_ids=0,v=3.0)[0]['uid']:
		api.messages.edit(peer_id=2000000000+main_message["items"][0]["chat_id"],message=new_message,message_id=message_id,v=APIVersion)
	else:
		api.messages.edit(peer_id=user_id,message=new_message,message_id=message_id,v=APIVersion)


def RemoveMessage(message_id):
	if api.messages.delete(message_ids=message_id,delete_for_all=1,v=APIVersion)[str(message_id)] == 1:
		print("Сообщение успешно удалено")

message_string = input("Введи сообщение для работы с ним:\n")
main_message = api.messages.search(q=message_string,preview_length=0,offset=0,count=1,v=APIVersion)
