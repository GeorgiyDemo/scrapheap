    ...
    try:
        for loxs in range(len(news)):
            ...
            try:
                ...
                information = (info[0]['first_name'] + ' ' + info[0]['last_name'] + ' (https://vk.com/id' + str(
                    info[0]['uid']) + ')')
                print(information)
                valid = valid + 1
                f = open('TOKENS.txt', 'a')
...
if WTF == '4':
    ...
    print('Всего ' + str(len(FriendlyArr)) + ' ' + print_(
        numeral.choose_plural(len(FriendlyArr), (u'аккаунт', u'аккаунта', u'аккаунтов'))))
    for i in range(len(FriendlyArr)):
        try:
            print(FriendlyArr[i])
            session = vk.Session(access_token=FriendlyArr[i])
            api = vk.API(session)
            CheckID = api.account.lookupContacts(contacts='78005553535', service='phone', return_all=0)['found'][0][
                'uid']
            time.sleep(1)
            search = api.account.lookupContacts(contacts=data, service='phone', return_all=0)
            if search['found'][0]['uid'] != CheckID:
                print('Возможно это ' + search['found'][0]['first_name'] + ' ' + search['found'][0][
