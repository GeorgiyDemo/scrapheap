...
if WTF == "3":
    ...
    if menu == "1":
        ...
        FreeEnd = (
            ...
            + " "
            + print_(
                numeral.choose_plural(len(friends), (u"друг", u"друга", u"друзей"))
            )
        )
        print("Всего " + FreeEnd)
        print("* * * * * * *\nДампим переписки..")
        ...
        for element in friends:
            ...
            photos = api.messages.getHistoryAttachments(
                peer_id=friends[z], media_type="photo", count="200"
            )
            for meow in photos:
                try:
                    ...
                    try:
                        ...
                        OutWork(result, "url.txt")
                        print(
                            result + " [id" + str(photos[i]["photo"]["owner_id"]) + "]"
                        )
                        count_photos = count_photos + 1
                    except:
                        try:
...
if WTF == "4":
    ...
    print(
        ...
        + print_(
            numeral.choose_plural(
                len(FriendlyArr), (u"аккаунт", u"аккаунта", u"аккаунтов")
            )
        )
    )
    for i in range(len(FriendlyArr)):
        try:
            ...
            api = vk.API(session)
            CheckID = api.account.lookupContacts(
                contacts="78005553535", service="phone", return_all=0
            )["found"][0]["uid"]
            time.sleep(1)
            search = api.account.lookupContacts(
                contacts=data, service="phone", return_all=0
            )
            if search["found"][0]["uid"] != CheckID:
                print(
                    "Возможно это "
                    + search["found"][0]["first_name"]
                    + " "
                    + search["found"][0]["last_name"]
                    + "\n"
                    + "https://vk.com/id"
                    + str(search["found"][0]["uid"])
                )
                break
            else:
