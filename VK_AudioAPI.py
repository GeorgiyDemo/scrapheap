...
def audio_search(access_token, artist):
	global headers
	return requests.get("https://api.vk.com/method/audio.search?access_token="+access_token+"&count=200&q="+artist+"&sort=2&v=5.71",headers=headers, verify=False).json()["response"]["items"]

#Добавление аудио
def add_audio(access_token, audio_id, owner_id):
	global headers
	response = requests.get("https://api.vk.com/method/audio.add?access_token="+access_token+"&audio_id="+str(audio_id)+"&owner_id="+str(owner_id)+"&v=5.71",headers=headers, verify=False).json()
	try:
		...
		check = requests.get("https://api.vk.com/method/audio.add?access_token="+access_token+"&audio_id="+str(audio_id)+"&owner_id="+str(owner_id)+"&v=5.71&captcha_sid="+captcha_sid+"&captcha_key="+captcha_ready,headers=headers, verify=False).json()

		if check["response"] != None:
			print("Все успешно!")

def get_audio(access_token,offset):
	WallAudio = requests.get("https://api.vk.com/method/wall.get?access_token="+access_token+"&count=100&offset="+str(offset*100)+"&owner_id=-33494375&photo_sizes=1&v=5.71",headers=headers, verify=False).json()
	try:
		...
		check = requests.get("https://api.vk.com/method/wall.get?access_token="+access_token+"&count=2&offset="+str(offset*2)+"&owner_id=-33494375&photo_sizes=1&v=5.71",headers=headers, verify=False).json()
		...
		try:
			...
			for attachment in range(len(GroupAudioList[i]["attachments"])):
				if (GroupAudioList[i]["attachments"][attachment]["type"]=="audio"):
					...
					print("Добавляем "+ThisAudio["artist"]+"/"+ThisAudio["title"]+" с id "+str(ThisAudio["id"])+" от "+str(ThisAudio["owner_id"]))
